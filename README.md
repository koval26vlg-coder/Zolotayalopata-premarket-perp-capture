...
& $py src\event_registry.py --upcoming --horizon-hours 48 --source-class VENUE_INSTRUMENT_METADATA
```

`--source-class` обязателен: какой класс `t0` считать основанием для capture — решение
вызывающего, и раньше CLI принимал его молча по умолчанию.

Реестр append-only: перенос `t0` дописывается ревизией с прежним значением в
`supersedes`, а не затирает его. Цепочка ревизий проверяется: она обязана начинаться с
нуля, а `supersedes` — называть значение, которое предыдущая ревизия действительно
держала. Цепочки разных классов источника не пересекаются. Refresh отчитывается о полноте — если курсор площадки
...
План неизменяем, и перевыпуск — это **новый файл**, а не тот же путь заново. Версия
входит в имя файла и в `plan_id`; предыдущие версии остаются на диске, а текущий план
несёт `supersedes` с их хешами. `--plan-check` проверяет родословную: удалённый или
отредактированный superseded-план роняет проверку.

Порядок: поднять `PLAN_VERSION` в `project_config.py`, добавить прежний путь в
`SUPERSEDED_PLAN_PATHS`, сгенерировать, перепинить trust-root, записать решение.

```powershell
& $py src\plan_builder.py --write-plan
```

Это правило было прозой в `AGENTS.md`, и план по одному пути перегенерировался трижды,
не оставив следов. См. `docs/decisions/004`.
